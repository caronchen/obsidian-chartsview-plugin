...
/**
 * From better-word-count by Luke Leppan (https://github.com/lukeleppan/better-word-count)
 * @returns 
 */
...
export function getWordCount(text: string, filter: string): WordCount[] {
    ...
    const wordCount: Array<WordCount> = [];
    const ignoreRegex = new RegExp(filter.split("\n").map(e => ['^', e.trim(), '$'].join('')).join('|'), "i");
    for (const word in words) {
        if (!ignoreRegex.test(word)) {
            wordCount.push({word: word, count: words[word]});
        }
    }
